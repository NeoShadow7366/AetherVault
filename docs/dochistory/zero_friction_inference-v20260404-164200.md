...
The inference stack responds directly to the `settings.json` configurations dictating the active pipeline endpoints. There are no exposed environment variable restrictions; it strictly uses Python `urllib` to enforce the Anti-Gravity 0-dependency mandate.
